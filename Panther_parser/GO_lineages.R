...
# Change factor order
GO_gp$Group <- factor(GO_gp$Group,levels = c("04_dpa_Down","04_dpa_Up","15_dpa_Down","15_dpa_Up"))
...
group.labs <- c(`04_dpa_Down` = "04_dpa\nDown\nregulated",
                `04_dpa_Up` = "04_dpa\nUp\nregulated",
                `15_dpa_Down` = "15_dpa\nDown\nregulated",
                `15_dpa_Up` = "15_dpa\nUp\nregulated")
...
fig2 <- ggplot(GO_gp, aes(x = GO, y = Fold_enrichment)) +
  ...
  guides(y = guide_axis(order=1),
         colour = guide_colourbar(order=1))

pdf(output_file_name,paper = "a4r")
fig2
dev.off()
