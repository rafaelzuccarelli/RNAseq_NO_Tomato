...
SNO_list = []
for line in input_SNO_file:
	line_list = list(line.split('\n'))
	SNO_list.append(line_list[0])

input_YNO2_file = wd + '/dic_files/Solyc_YNO2'
YNO2_list = []
for line in input_YNO2_file:
	line_list = list(line.split('\n'))
	YNO2_list.append(line_list[0])
...
for currentpath, folders, files in os.walk(input_directory):
	for file in files:
		...
		for line in input_DE: #add features checking each Solyc, line by line
